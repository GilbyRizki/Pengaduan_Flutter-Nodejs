...
class MasyarakatController extends GetxController {
  ...
  void getData() async {
    try {
      ...
      var res =
          await axios.get(Uri.parse('http://192.168.232.42:5000/masyarakat'));
      print(res);
  ...
  Future<bool> postData() async {
    ...
    final response = await axios.post(
        Uri.parse('http://192.168.232.42:5000/masyarakat'),
  ...
  Future<bool> updateData(nik) async {
    ...
    final response = await axios.patch(
        Uri.parse('http://192.168.232.42:5000/masyarakat/${nik}'),
        headers: {"content-type": "application/json"},
        body: jsonEncode(datas));
    if (response.statusCode == 201) {
      // ignore: invalid_use_of_protected_member
